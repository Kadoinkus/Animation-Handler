...
import { EMOTION_TABLE, STATE_TRANSITIONS } from './constants/emotions';
import FileDropZone from './components/FileDropZone';

/* === 1. Find complete emotion packs ============================= */
...
export default function App() {
  ...
  const go = (target, code = '') => {
    if (!STATE_TRANSITIONS[state].includes(target)) return;
  ...
  const canSleep = next.includes('sleep');
  const canWait = next.includes('wait');
  const canReact = next.includes('react');
  const canType = next.includes('type');
  ...
  return (
    <div style={{ padding: '20px' }}>
      ...
      <div className="row">
        ...
        {!playing && queue.length > 0 && <Btn label="▶ Resume" enabled onClick={resume} />}
      </div>
      ...
      {show && (
        <div className="pop">
          {/* Default first */}
          <div className="pop-item" onClick={() => go('type', '')}>
            Default
          </div>

          {/* dynamic emotion list */}
          {emotions.map((e) => (
            <div key={e.code} className="pop-item" onClick={() => go('type', e.code)}>
              {e.label}
            </div>
          ))}
          <div
